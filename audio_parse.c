...
{
#ifdef DEBUG
	fprintf(stderr,"target: %d   src: %d  length: %d \n",target,src,length);
#endif
...
uint64_t add_pts_audio(uint64_t pts, audio_frame_t *aframe, uint32_t frames)
{
	uint64_t newpts=0;
	uint64_t samp = 0;
	...
	newpts = (pts + (frames *samp * 90000UL) 
		  / aframe->frequency);
...
int check_audio_header(uint8_t *inbuf, audio_frame_t *af, long  off, int le)
{
	...
	case MPEG_AUDIO:
 	...
 	case AAC:
		af->framesize = ((headr[3] & 0x03) << 12) | (headr[4] << 3) | (headr[5] >> 5); 
		af->mpg25 = 1024*(((headr[7] & 0xc0)>> 2)+1); 
		fr = (headr[2] & 0x3c) >> 2;
...
int get_mpeg12_info(uint8_t *inbuf, audio_frame_t *af, long off, int le) 
{
	int c = 0;
	int fr =0;
	uint8_t *headr,mode;
...
int get_ac3_info(uint8_t *inbuf, audio_frame_t *af, long off, int le)
{
	int c=0;
	uint8_t *headr;
	uint8_t frame;
...
int get_aac_info(uint8_t *inbuf, audio_frame_t *af, long off, int le)
{
...
int find_first_sync(uint8_t *inbuf, int le,
		    audio_frame_t *af, int off)
{
	...
	if (!af->spos){
		...
		int l = minb - af->spos;
		if ( le >= l){
			mycopy(af->syncbuf+af->spos, inbuf, l);
			c = get_info(af->syncbuf, af, 0, 
				     af->spos+l);
		} else {
			mycopy (af->syncbuf+af->spos, inbuf, le);
			af->spos+=le;
			c = -1;
		}
		return c;
...
// check audio header of next frame (also check for mpeg padding)
		...
		if (fdiff < minb){
			if (len > minb){
				if (c < olen){
					mycopy(af->mainbuf+af->mainpos, buf+c, minb );
				} else {
					af->set = 0;
					af->mainpos = 0;
					af->lastframe = 0;
			...
			af->mainpos += minb;
				...
				if (c < olen){
					mycopy(af->mainbuf+af->mainpos, buf+c, len);
				} else {
					af->set = 0;
					af->mainpos = 0;
					af->lastframe = 0;
		...
		padlen = ((flen +15) &~15) - flen; 
		if (af->mainsize - af->mainpos < flen + padlen){
			memmove(af->mainbuf, af->mainbuf+af->lastframe,fdiff);
			af->mainpos = 0;
			af->lastframe = 0;
			af->mainpos = fdiff;
		}
		...
		if ( len > flen - fdiff){
			if (c < olen && flen > fdiff){
				mycopy(af->mainbuf+af->mainpos, buf+c, flen -fdiff);
				af->mainpos += flen - fdiff;
				len -= flen - fdiff;
				c += flen - fdiff;
				fdiff = 0;
				af->mainpos += padlen;
				...
				af->lastframe += af->framesize + padlen;  
				if (af->mainsize - af->mainpos < flen + padlen){
					af->mainpos = 0;
					af->lastframe = 0;
				}
